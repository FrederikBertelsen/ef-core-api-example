...
public class CustomerRepository(AppDbContext dbContext) : ICustomerRepository
{
    ...
    public async Task<Customer?> GetCustomerByIdAsync(Guid customerId) => await dbContext.Customers.Include(customer => customer.Orders).FirstOrDefaultAsync(customer => customer.Id == customerId);
    public async Task<Customer?> GetCustomerByEmailAsync(string email) => await dbContext.Customers.Include(customer => customer.Orders).FirstOrDefaultAsync(c => c.Email == email);
